...
if [ -z "$1" ]; then
    ...
    echo "3. Name: 'Game of Life TUI Development'"
...
if [[ ! $TOKEN =~ ^(ghp_|github_pat_) ]]; then
    ...
    echo "GitHub Personal Access Tokens should start with 'ghp_' or 'github_pat_'"
    exit 1
fi
...
echo "  ./scripts/git-push.sh   - Push to GitHub"
echo "  ./scripts/git-pull.sh   - Pull from GitHub"
echo "  ./scripts/git-sync.sh   - Pull and push in one command"
echo ""
